...
div.container
  ...
  div#loadHistory_mask(style='display:none')
    p.help-block.small.text-right 会员卡(旧)的充值记录，包括每次课时的变更(增加)，不包括课时消耗的记录，如果需要查询课时的销课记录，请查询“上课记录”
  ...
  methods: {
    ...
    linkNameFormatter: function(value, row, index) {
      return [
        ...
        ' <i class="text-primary glyphicon glyphicon-blackboard me-3"></i>' + value,
        '</a>'
      ].join('');
    },
