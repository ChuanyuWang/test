...
v-container
  ...
  v-data-table(:headers="headers" :items="rawData" :items-per-page="10" :server-items-length="totalNotice" 
    ...
    template(v-slot:item.actions="{ item }")
      v-btn.me-1(small v-if="item.status==='open'" @click.stop="openEditNoticeDialog(item)") 编辑
      v-btn.me-1(small @click.stop="noticeID=item._id,deleteNoticeDialog=true") 删除
      v-btn(small v-if="item.status==='open'" @click.stop="noticeID=item._id,publishNoticeDialog=true") 发布
